...
export default function StudioClient({ connectionId }: { connectionId?: string }) {
  const { data: session } = authClient.useSession();
  const userKey = session?.user?.id ?? "guest";
  const router = useRouter();

  const [connections, setConnections] = useState<any[]>([]);
  const [selectedConn, setSelectedConn] = useState(connectionId ?? DEMO_ID);
  ...
  useEffect(() => {
    const fetchConns = async () => {
      ...
      setConnections([
        { id: DEMO_ID, name: "✨ Demo eCommerce DB" }, 
        ...userConns.filter(c => c.id !== DEMO_ID)
      ]);
    };
    fetchConns();
  }, [session]);
  ...
  const renderDynamicChart = (type: string, labelKey: string, valueKey: string) => {
    ...
    if (results.length === 1) {
      // Single row with multiple aggregate columns (e.g. total_customers, total_orders)
      const firstRow = results[0];
      const keys = Object.keys(firstRow);
      data = keys
        .filter((key) => {
          const val = firstRow[key];
          if (key.toLowerCase().includes("id") && typeof val === "string") return false;
      ...
      const firstRow = results[0];
      const keys = Object.keys(firstRow);
      let activeLabelKey = labelKey;
      let activeValueKey = valueKey;

      if (!keys.includes(labelKey)) {
        // Fallback: first non-numeric/string key, or first key
        const strKey = keys.find((k) => isNaN(Number(firstRow[k])));
        activeLabelKey = strKey || keys[0] || "";
      }
      if (!keys.includes(valueKey)) {
        // Fallback: first numeric key, or second key
        const numKey = keys.find((k) => !isNaN(Number(firstRow[k])) && firstRow[k] !== null && firstRow[k] !== "");
        activeValueKey = numKey || keys[1] || keys[0] || "";
      }

      data = results.slice(0, 15).map((row, i) => ({
        name: String(row[activeLabelKey] ?? `Row ${i + 1}`).slice(0, 20),
        value: Number(row[activeValueKey]) || 0,
      }));
    }
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              router.push(`/dashboard/studio/${encodeURIComponent(val)}`);
            }}
          >
            {connections.map((c) => (
        ...
        <div className="flex-1 grid grid-cols-1 xl:grid-cols-[260px_1fr] gap-4 min-h-0 overflow-hidden">
          ...
          <div className="flex flex-col gap-4 overflow-hidden order-1 xl:order-2 h-full min-h-0">
            <Card className="p-4 space-y-3 border-primary/20 shrink-0">
              ...
              <Button onClick={generateSql} disabled={isGenerating || isRunning || !naturalQuery.trim() || !selectedConn} className="w-full">
